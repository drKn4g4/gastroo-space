// src/app/api/workspace/callback/route.ts
...
<head>
  <meta charset="utf-8">
  <title>Google Workspace – autoryzacja</title>
</head>
<body>
  <script>
    ${
      error
        ? `window.opener?.postMessage(
            { type: 'WORKSPACE_AUTH_ERROR', error: ${JSON.stringify(error)} },
            window.location.origin
          );`
        : `window.opener?.postMessage(
            { type: 'WORKSPACE_AUTH_SUCCESS', code: ${JSON.stringify(code)}, state: ${JSON.stringify(state)} },
            window.location.origin
          );`
    }
    window.close();
